...
def plot_trades(price_data, trades):
    '''
    Creates a plot of the price with coloured bands to show each trade and poition
    '''
    ...
    for index, long_position in trades.loc[trades['position'] == 1].iterrows():
    ...
    for index, short_position in trades.loc[trades['position'] == -1].iterrows():

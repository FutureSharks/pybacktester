...
def apply(price_data, moving_average, ma_bandwidth_pips):
    '''
    Moving average band strategy
    '''
    price_data['ma'] = price_data['price'].rolling(window=moving_average, center=False).mean()
    price_data['ma_high'] = price_data['ma'] + (0.0001 * ma_bandwidth_pips)
    price_data['ma_low'] = price_data['ma'] - (0.0001 * ma_bandwidth_pips)
